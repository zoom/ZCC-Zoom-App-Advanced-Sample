...
function App() {
  ...
  const [showZoomApps, setShowZoomApps] = useState(false);
  ...
  return (
    <div className="App">
      <Engagement configureSdk={configureSdk} />
      <br />
      <button onClick={() => setShowZoomApps(!showZoomApps)}>{showZoomApps ? `Hide` : `Show`} Zoom Apps APIs</button>

      {showZoomApps && <div><h1>Hello{user ? ` ${user.first_name} ${user.last_name}` : " Zoom Apps user"}!</h1>
